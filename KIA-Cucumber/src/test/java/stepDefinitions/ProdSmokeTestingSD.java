...
public class ProdSmokeTestingSD {
	...
	public void user_is_on_KIA_web_portal_Landing_Page() {
		...
		driver.get("https://owners.kia.com/content/owners/en/kia-owner-portal.html/");
	}
...
//If in case if we have more than one vehicle in the account, then we should enable those 2 methods below.
...
/*	@Then("^user is on welcome page$")
	...
	java.util.List<WebElement> vehList = driver.findElements(By.xpath("//div[@class='my-vehicles-grid-item-header']/strong"));
	for (WebElement vehlst : vehList) {
		...
		if(totVehlst.equalsIgnoreCase("My SELTOS")){
			driver.findElement(By.xpath("//img[@title='SELTOS']")).click();  
			break;
		} 
		Thread.sleep(10000);
	}
	...
	*/
	...
	public void user_clicks_the_lockUnlock_option() throws InterruptedException  {
			...
			for (int i = 0; i <=1; i++) {
				...
				if (rdStatus.equalsIgnoreCase("DOORS ARE LOCKED")) {
					driver.findElement(By.xpath("//span[@class='action-button state-1 btn-unlock']")).click();
				} else if(rdStatus.equalsIgnoreCase("DOORS ARE UNLOCKED")) {
					driver.findElement(By.xpath("//span[@class='action-button state-2 btn-lock']")).click();
				} 
				Thread.sleep(60000);
	...
	public void user_clicks_the_remoteClimate_start_option() throws InterruptedException {
			...
			String rdCliStatus = driver.findElement(By.xpath("(//span[@class='state-1 active'])[2]")).getText();
