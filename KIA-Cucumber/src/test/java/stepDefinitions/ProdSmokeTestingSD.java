...
public class ProdSmokeTestingSD {
	...
	for (WebElement vehlst : vehList) {
		...
		if(totVehlst.equalsIgnoreCase("My NIRO PHEV")){
			driver.findElement(By.xpath("(//img[@title='NIRO PHEV'])[1]")).click();  
			break;
		} 
		Thread.sleep(10000);
	}
	...
	public void user_clicks_the_remoteClimate_start_option() throws InterruptedException {
			...
			System.out.println("Current Actions is = "+ rdCliStatus);
				...
				for (int i = 0; i <=1; i++) {
					...
					String cliStat = null;
					try {
						driver.findElement(By.xpath("//span[@class='ui-dial-temperateure']")).click();
